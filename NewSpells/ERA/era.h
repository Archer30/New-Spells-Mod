...
// The ERA exports copy the file name into a caller-owned buffer.  The Pascal
// SDK and the ERA changelog are authoritative here; older C headers incorrectly
// described these procedures as functions returning an allocated era_str.
ERA_API void (__stdcall *GetCampaignFileName) (char* Buf);
ERA_API void (__stdcall *GetMapFileName) (char* Buf);
